...
import mrtjp.projectred.fabrication.operations.{CircuitOp, CircuitOpErase, OpGateCommons, OpWire, SimplePlacementOp}
...
class PrefboardNode(circuit: IntegratedCircuit, hasBlueprint: Boolean, previewUpdateDelegate: (CircuitOp) => Unit) extends TNode {
    ...
    if(hasBlueprint) {
      ...
      if (currentOp != null) {
        if (rayTest(mouse) && !leftMouseDown) {
          // Render: Erase always, everything else only if there aren't placed parts under the cursor
          currentOp match {
            case _: OpGateCommons | _: OpWire | _: SimplePlacementOp =>
              if (circuit.getPart(toGridPoint(mouse.vectorize)) == null)
                currentOp.renderHover(circuit, toGridPoint(mouse).vectorize, scale, offset)
  ...
  override def mouseReleased_Impl(p: Point, button: Int, consumed: Boolean) = {
    if (leftMouseDown) {
      ...
      val opUsed = currentOp != null && circuit.sendOpUse(
        currentOp,
        currentOp.getRotation(),
        currentOp.getConfiguration(),
        mouseStart,
        mouseEnd
      )
  ...
  override def mouseScrolled_Impl(p: Point, dir: Int, consumed: Boolean) = {
    if (!consumed && rayTest(p)) {
      if (dir > 0) rescaleAt(p, scale * 1.25)
      else if (dir < 0) rescaleAt(p, scale * 0.8)
      true
    } else false
  }
  ...
  def incScale() {
    rescaleAt(parent.frame.midPoint, scale * 1.25)
  }

  def decScale() {
    rescaleAt(parent.frame.midPoint, scale * 0.8)
  }
